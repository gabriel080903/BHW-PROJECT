...
export default function Login({ onLogin }) {
  ...
  // Google OAuth — uses real backend
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        // Send Google token to backend for verification + session creation
        try {
          ...
          // Backend Google token endpoint not yet wired — fall through to local session
        }

        // Fallback: local session for Parent/Guardian role
        ...
        // Register parent locally for admin assignment feature
        try {
          const known = JSON.parse(localStorage.getItem('bhw_known_parents') || '[]')
          if (!known.find(p => p.id === session.id)) {
            known.push({ id: session.id, name: session.username, email: session.email })
            localStorage.setItem('bhw_known_parents', JSON.stringify(known))
          }
        } catch { /* ignore */ }
  ...
  // Authenticates against the real backend — no hardcoded passwords
          ...
          {hasGoogleAuth && (
            <>
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className="w-full flex items-center justify-center gap-3 bg-white hover:bg-gray-50 text-gray-700 font-bold py-3 rounded-xl text-sm shadow-md transition-all disabled:opacity-60 disabled:cursor-not-allowed mb-4"
              >
                {googleLoading ? (
